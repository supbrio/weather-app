...
const throwError = function(error){
    body.insertAdjacentHTML('afterbegin',error)
}
...
const getWeather = async function(country){
    try{
        // Get Country
        const resCoords = await fetch(`https://restcountries.com/v2/name/${country}`);
        if(!resCoords.ok) return throwError('<p class="error-message">Country not found!</p>')
        ...
        // Get weather
        const resWeather = await fetch(`http://www.7timer.info/bin/api.pl?lon=${lng}&lat=${lat}&product=civil&output=json`);
        ...
        <p class="temperature-data">${dataWeather.dataseries[0].temp2m > 1 ? '<ion-icon name="sunny-outline" class="icon sunny"></ion-icon>' : '<ion-icon name="snow-outline" class="icon cold"></ion-icon>'}${dataWeather.dataseries[0].temp2m}&deg;C</p>
        <p>09.03.2022</p>
